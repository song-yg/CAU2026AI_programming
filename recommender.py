...
'''함수들이 클래스 밖에 있는 이유는 main에서 부분부분 뽑아쓰려고'''
...
# 냉장고에 필요한 재료가 있는지 검사한다.
def check_required_item(fridge_manager, required):

    name = required["name"]
    amount = required.get("amount") #필요 수량 (없으면 None)
    unit = required.get("unit") #필요 단위 (없으면 None)

    if name in ALWAYS_AVAILABLE: #물

        return True, ""
    
    #batches = 냉장고에 한 재료가 여러번 저장된경우 그 재료의 여러 저장된 양을 합쳐야할 수도 있어서 냉장고에 저장된 재료의 목록을 가져옴
